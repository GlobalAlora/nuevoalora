...
export default async function CallBookedPage({ params }: Props) {
  ...
  const steps = l === "es"
    ? [
        { icon: "📅", title: "Tu llamada quedó agendada", desc: "Ya tenés el horario reservado, te llega la confirmación por mail." },
        { icon: "🧭", title: "La preparamos", desc: "Revisamos tu proyecto antes de la llamada para aprovechar bien el tiempo." },
        { icon: "🎥", title: "Nos conectamos", desc: "20 minutos para entender tu caso y mostrarte cómo podemos ayudarte." },
      ]
    : [
        { icon: "📅", title: "Your call is booked", desc: "Your time slot is reserved, a confirmation is on its way to your inbox." },
        { icon: "🧭", title: "We prepare for it", desc: "We review your project beforehand to make the most of the call." },
        { icon: "🎥", title: "We hop on a call", desc: "20 minutes to understand your case and show you how we can help." },
      ];
        ...
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-[1fr_1.1fr] lg:gap-16">
          ...
          <div className="flex flex-col gap-5">
            ...
            <SpotlightCard
              accent="#25D366"
              className="rounded-2xl border p-6"
              style={{ borderColor: "rgba(255,255,255,0.08)", background: "linear-gradient(160deg, rgba(37,211,102,0.08), rgba(255,255,255,0.015))" }}
            >
